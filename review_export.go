...
func writeReviewWorkbook(target string, rule *importer.Rule, rows []store.Row) error {
	workbook := excelize.NewFile()
	defer workbook.Close()
	const sheet = "Sheet1"
	for index, column := range rule.Columns {
		cell, err := excelize.CoordinatesToCellName(index+1, 1)
		if err != nil {
			return err
		}
		if err := workbook.SetCellStr(sheet, cell, column.ChineseName); err != nil {
			return err
		}
	}
	...
	for rowIndex, row := range rows {
		for columnIndex, column := range rule.Columns {
			cell, err := excelize.CoordinatesToCellName(columnIndex+1, rowIndex+2)
			if err != nil {
				return err
			}
			if err := workbook.SetCellStr(sheet, cell, row[column.DBField]); err != nil {
				return err
			}
